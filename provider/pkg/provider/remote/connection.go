...
const (
	...
	DefaultDialErrorLimit = 10
	UnlimitedDialErrors   = -1
)
...
func (c *Connection) Annotate(a infer.Annotator) {
	...
	a.SetDefault(&c.DialErrorLimit, DefaultDialErrorLimit)
}
	...
	_, _, err = retry.Until(ctx, retry.Acceptor{
		Accept: func(try int, nextRetryTime time.Duration) (bool, interface{}, error) {
			...
			if err != nil {
				...
				dials := try + 1
				if *con.DialErrorLimit > UnlimitedDialErrors && dials > *con.DialErrorLimit {
					return true, nil, err
				}
